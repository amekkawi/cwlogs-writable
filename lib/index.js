 ...
 * @param {object} [options.cloudWatchLogsOptions={}] - Options passed when creating [AWS.CloudWatchLogs](http://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/CloudWatchLogs.html#constructor-property) service.
...
CWLogsWritable.prototype.nextLogBatchSize = function(queuedLogs) {
	...
	for (var i = 0, l = queuedLogs.length; i < l; i++) {
		...
		if (sizeEstimate > this.maxBatchSize || batchCount >= 10000) {
			break;
		}
		else {
			batchCount = i + 1;
...
/**
 * Send the next batch of log events to AWS CloudWatch.
 *
 * @private
 * @returns {void}
 */
...
/**
 * Attempt to continue sending log events to AWS CloudWatch after an error was previously returned.
 *
...
/**
 * Send a PutLogEvents action to AWS CloudWatch.
 *
...
/**
 * Describe the LogStream in AWS CloudWatch to get the next sequence token.
 *
 * @param {function} cb
 * @private
 */
...
/**
 * Create both the LogGroup and LogStream in AWS CloudWatch.
 *
 * @param {function} cb
 * @private
 */
...
/**
 * Create the LogGroup in AWS CloudWatch.
 *
 * @param {function} cb
 * @private
 */
...
/**
 * Create the LogStream in AWS CloudWatch.
 *
 * @param {function} cb
 * @private
 */
